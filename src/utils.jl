...
differential equation problem for variable `u0` with algorithm `alg`.
"""
...
Return the absolute tolerance for solving the differential equation problem with variable `u`
and time span `tspan` with algorithm `alg`.
"""
...
Return the relative tolerance for solving the differential equation problem with variable `u`
and time span `tspan` with algorithm `alg`.
"""
...
"""
    rate_prototype_of(u, tspan)

Return prototype of rates for a differential equation problem with state `u` and time span `tspan`.
"""
rate_prototype_of(u, tspan) =
  DiffEqBase.@.. u * $(inv(oneunit(eltype(tspan))))
...
"""
function solution_arrays(u, tspan, rate_prototype = rate_prototype_of(u, tspan);
