...
differential equation problem for initial state `u0` with algorithm `alg`.
"""
function u_uprev_uprev2(u0, alg; allow_extrapolation = alg_extrapolates(alg), kwargs...)
...
function callback_set_and_cache(prob, callback)
    ...
    if max_len_cb isa VectorContinuousCallback
        ...
        callback_cache =
            DiffEqBase.CallbackCache(max_len_cb.len, uBottomEltype, uBottomEltype)
    else
...
function build_history_function(
    ...
    ode_ts, ode_timeseries, ode_ks = solution_arrays(
        ode_u,
        tspan,
        rate_prototype;
        timeseries_init = (),
    ...
    ode_cache = OrdinaryDiffEq.alg_cache(
        alg.alg,
        ode_u,
        rate_prototype,
        uEltypeNoUnits,
    ...
    if iscomposite(alg)
        ode_alg_choice = Int[]
        ode_id = OrdinaryDiffEq.CompositeInterpolationData(
            ode_f,
            ode_timeseries,
            ode_ts,
            ode_ks,
        ...
        ode_sol = DiffEqBase.build_solution(
            ode_prob,
            alg.alg,
            ode_ts,
            ode_timeseries;
        ...
        ode_id = OrdinaryDiffEq.InterpolationData(
            ode_f,
            ode_timeseries,
            ode_ts,
            ode_ks,
