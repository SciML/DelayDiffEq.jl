...
function DiffEqBase.savevalues!(
    integrator::HistoryODEIntegrator,
    force_save = false,
    reduce_size = false,
)::Tuple{Bool,Bool}
...
function DiffEqBase.savevalues!(
    ...
    # update history
    saved, savedexactly = DiffEqBase.savevalues!(ode_integrator, force_save, false) # reduce_size = false
...
function OrdinaryDiffEq.initialize!(integrator::DDEIntegrator)
    ...
    @inbounds for i = 1:length(integrator.k)
...
function DiffEqBase.reinit!(
    ...
    integrator.opts.d_discontinuities = OrdinaryDiffEq.initialize_d_discontinuities(
    ...
    tstops_propagated, d_discontinuities_propagated =
        initialize_tstops_d_discontinuities_propagated(
            tType,
            tstops,
            d_discontinuities,
            tspan,
    ...
    if erase_sol
        ...
        if order_discontinuity_t0 ≤ OrdinaryDiffEq.alg_maximum_order(integrator.alg)
            ...
            integrator.tracked_discontinuities[1] =
                Discontinuity(integrator.tdir * integrator.t, order_discontinuity_t0)
        else
...
function DiffEqBase.add_tstop!(integrator::DDEIntegrator, t)
    integrator.tdir * (t - integrator.t) < 0 && error(
...
DiffEqBase.addsteps!(integrator::DDEIntegrator, args...) =
...
function DiffEqBase.reeval_internals_due_to_modification!(
    integrator::DDEIntegrator,
    x::Type{Val{not_initialization}} = Val{true},
