function DiffEqBase.__solve(
    prob::DiffEqBase.AbstractDDEProblem,
...
function DiffEqBase.__init(
    ...
    # create solution
    if iscomposite(alg)
        id = OrdinaryDiffEq.CompositeInterpolationData(
            f_with_history,
            timeseries,
            ts,
            ks,
        ...
        sol = DiffEqBase.build_solution(
            ...
            k = ks,
            interp = id,
            alg_choice = id.alg_choice,
            calculate_error = false,
            destats = destats,
        )
    else
        id = OrdinaryDiffEq.InterpolationData(
            f_with_history,
            timeseries,
            ts,
            ks,
    ...
    integrator = DDEIntegrator{
        ...
        typeof(k),
        typeof(sol),
        typeof(f_with_history),
        typeof(cache),
        typeof(ode_integrator),
        typeof(fpsolver),
        typeof(opts),
        typeof(discontinuity_abstol),
...
function OrdinaryDiffEq.initialize_callbacks!(
    integrator::DDEIntegrator,
    initialize_save = true,
)
